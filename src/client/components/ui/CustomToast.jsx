...
const CustomToast = ({ message, type, fillWidth, autoHide = false, duration = 4000, visible, onHide }) => {
  ...
  return visible && message && (
    <div className={`${type === "error" ? "border-red-500 bg-red-50" : type === "success" ? "border-green-500 bg-green-50" : type === "warn" ? "border-gray-500 bg-gray-50" : type === "info" && "border-blue-500 bg-blue-50"} ${fillWidth && 'w-full'} py-3 flex gap-2 justify-between border px-3 rounded-lg`}>
      <div className=''>
        {type === "error" ? (
      ...
      <p className={`${type === "error" ? 'text-red-500' : type === 'success' ? 'text-green-600' : type === 'info' ? 'text-blue-500' : 'text-gray-500'} w-[80%] text-sm max-w-[35vw] whitespace-wrap overflow-hidden text-ellipsis`}>
        {message}
      </p>
